...
class ReviewBoundaryTests(unittest.TestCase):
    def test_full_and_markup_only_generation_stay_local(self):
        with tempfile.TemporaryDirectory(prefix='murderbird-review-test-') as temporary:
            self.assert_review_boundary(Path(temporary))

    def test_canonical_root_generation_stays_local(self):
        with tempfile.TemporaryDirectory(prefix='murderbird-review-canonical-') as temporary:
            self.assert_review_boundary(Path(temporary).resolve())

    def test_symlink_root_links_reach_the_intended_files(self):
        with tempfile.TemporaryDirectory(prefix='murderbird-review-alias-') as temporary:
            base = Path(temporary).resolve()
            canonical = base / 'nested' / 'canonical'
            ...
            self.assert_review_boundary(alias)

    def assert_review_boundary(self, fixture_root):
        spec = importlib.util.spec_from_file_location('review_builder', REPO / 'assets/murderbird/v2/build-library.py')
        builder = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(builder)
        # Match the builder's __file__.resolve() root contract. Temporary roots
        # can contain aliases such as macOS /var pointing to /private/var.
        # Adapt the fixture instead of changing hash-preserved archive bytes.
        repo = Path(fixture_root).resolve()
            ...
            for page in review.rglob('*.html'):
                ...
                for node in soup.select('[src], [href], [srcset]'):
                    ...
                    for url in urls:
                        ...
                        self.assertTrue(destination.is_file(), (page, url))
                        self.assertTrue(destination.is_relative_to(repo), (page, url))
                manifest_link = soup.find('a', string='Asset metadata')
                if manifest_link:
                    self.assertEqual((page.parent / manifest_link['href']).resolve(), root / 'manifest.json')
                for node in soup.select('img[src]'):
                    self.assertIn((page.parent / node['src']).resolve(), image_before)
