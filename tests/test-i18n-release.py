...
class I18nReleaseTests(unittest.TestCase):
    ...
    def test_regional_source_hash_ignores_generated_csp_but_not_visible_content(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            original = root / "original.html"
            regenerated = root / "regenerated.html"
            changed = root / "changed.html"
            original.write_text('<meta http-equiv="Content-Security-Policy" content="one"><p>stable</p>\n', encoding="utf-8")
            regenerated.write_text('<meta content="two" http-equiv="Content-Security-Policy"><p>stable</p>\r\n', encoding="utf-8", newline="")
            changed.write_text('<meta http-equiv="Content-Security-Policy" content="two"><p>changed</p>\n', encoding="utf-8")
            self.assertEqual(
