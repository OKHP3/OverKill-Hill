...
class ReviewedTargetIntegrityTests(unittest.TestCase):
    def setUp(self):
        ...
        self.config = {
            "schema_version": "1.0", "blocking_locales": ["fr"],
            "in_scope_routes": ["/", "/about/"],
            "target_locales": {
        ...
        for route in ("index.html", "about/index.html"):
            for prefix in ("", "fr/", "de/"):
                ...
                path.write_text('<meta http-equiv="Content-Security-Policy" content="one">'
        ...
        (self.root / "i18n/sync.config.json").write_text(json.dumps(self.config))
    ...
    def review(self):
        record = {
            "language_pair": "en-US -> fr-FR", "review_status": "ai-reviewed",
            "native_or_human_approval": False,
            "routes": [{"route": "/", "locale": "fr", "target_path": "fr/index.html",
    ...
    def test_target_only_change_blocks_without_changing_source_freshness(self):
        self.target.write_text("<p>changed translation</p>")
    ...
    def test_draft_target_change_is_advisory(self):
        (self.root / "de/index.html").write_text("changed")
        result = MODULE.load_results(self.config)
        self.assertEqual([], result["policy"]["blocking_items"])
    ...
    def test_generated_metadata_and_semantic_edits_require_review(self):
        ...
        for before, after in (("content=\"one\"", "content=\"two\""),
                              ("theme.css?v=11111111", "theme.css?v=22222222"),
                              ("a b", "ab"), ("<p>", "<p title=\"new\">"),
                              ("<p>a b ?v=11111111", "<p>a b ?v=22222222")):
            with self.subTest(edit=after):
                self.target.write_text(original.replace(before, after))
    ...
    def test_reviewed_target_only_adoption_updates_only_selected_pair(self):
        ...
        self.provenance.write_text(json.dumps(self.review()))
    ...
    def test_invalid_review_never_writes_ledger(self):
        self.target.write_text("<p>changed</p>")
        cases = [("route", "/wrong/"), ("locale", "de"), ("target_path", "de/index.html"),
                 ("source_sha256", "wrong"), ("target_sha256", "wrong"),
                 ("disposition", "rejected"), ("review_status", "rejected"),
                 ("language_pair", "en-US -> de-DE"), ("native_or_human_approval", True)]
    ...
    def test_source_and_target_drift_are_reported_independently(self):
        (self.root / "index.html").write_text("new source")
        self.target.write_text("new target")
        result = MODULE.load_results(self.config)
