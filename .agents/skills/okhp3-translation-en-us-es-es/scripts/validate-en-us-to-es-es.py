#!/usr/bin/env python3
"""Validate one en-US to es-ES text-artifact manifest and optional pair.
...
def validate_dictionary(dictionary: Any) -> List[str]:
    ...
    if not isinstance(pair, dict) or pair.get("source_locale") != SOURCE_LOCALE or pair.get("target_locale") != TARGET_LOCALE or pair.get("direction") != "one-way":
        add_error(errors, "target.dictionary must declare exactly the en-US to es-ES one-way pair")
...
def validate_manifest(project: Any, path: Path) -> List[str]:
    ...
    if not isinstance(target, dict):
        add_error(errors, "target must be one object for the es-ES output")
    else:
    ...
    if "targets" in project:
        add_error(errors, "targets is not allowed; this package permits exactly one es-ES output")
...
def compare_pair(source_path: Path, target_path: Path, preserve_urls: bool) -> Tuple[List[str], List[str]]:
    ...
    if not target_path.is_file():
        add_error(errors, f"missing es-ES target file: {target_path}")
        return errors, warnings
    ...
    if source == target:
        warnings.append("target is byte-identical to source; confirm that Spain Spanish translation was performed")
    if not target.strip():
        add_error(errors, "es-ES target file is empty")
    return errors, warnings
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--target-file", type=Path, help="optional es-ES target artifact to compare")
