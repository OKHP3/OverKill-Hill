...
Three modes:
  ...
  --mode check   Same as report, but exits 1 for missing or source-stale
                 routes in any configured locale. Never writes.
  --mode adopt   Write source and target hashes to the ledger for existing
                 targets needing a baseline or having a stale source hash.
                 Use after review; pass --routes to select reviewed routes.
                 Already in-sync pairs are not refreshed. Never edits page
                 content. Use a consuming site's adoption wrapper when it
                 requires provenance or other site-specific policy.
...
def sha256_file(path: Path) -> str:
    """Hash textual page content independently of the checkout line ending."""
