# en-US to es-MX text-artifact contract

This package has one language pair and one direction. The source is plainspoken
`en-US` and the output is `es-MX`. A manifest with another source, target,
...
```json
{
  "language_pair": {"source_locale": "en-US", "target_locale": "es-MX", "direction": "one-way"},
  "source": {
  ...
  "target": {"path": "content/es-mx/about.md", "sha256": "..."},
  "artifact_type": "markdown",
  "voice_profile": "config/voice-profile.en-us.json@1.0.0",
  "dictionary": "config/dictionary.en-us-es-mx.json@1.0.0",
...
has a declared es-MX equivalent:
...
- Markdown and HTML link targets remain unchanged unless the project contains
  an explicit es-MX route mapping;
...
Structural parity is not character-count parity. Mexican Spanish may expand or
contract. Do not force unnatural Mexican Spanish to match English length.
...
and claims. Format a value for es-MX only when the project rule says it is safe
...
3. preserve reviewed es-MX units whose source counterpart did not change;
4. translate only changed or new units using the current profile and dictionary;
5. mark the es-MX target stale until checks and review gates pass; and
6. retain the old target or a reviewable diff according to the consuming
   repository's normal version-control policy.

Do not regenerate the whole Mexican Spanish artifact if that would erase a reviewed Mexican Spanish
edit outside the changed source units.

## Publication boundary

A Mexican Spanish artifact does not prove that its route works, its canonical is correct,
