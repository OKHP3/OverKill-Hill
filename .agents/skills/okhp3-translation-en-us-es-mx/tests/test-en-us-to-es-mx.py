...
VALIDATOR = PACKAGE / "scripts" / "validate-en-us-to-es-mx.py"
PLANNER = PACKAGE / "scripts" / "plan-en-us-to-es-mx.py"


PROJECT = {
    "schema_version": "2.0",
    "project_id": "fixture-en-us-es-mx",
    ...
    "target": {"locale": "es-MX", "root": "content/es-mx", "dictionary": "config/dictionary.en-us-es-mx.json", "status": "draft", "needs_native_review": True},
...
DICTIONARY = {
    "schema_version": "1.0",
    "language_pair": {"source_locale": "en-US", "target_locale": "es-MX", "direction": "one-way"},
    "entries": [{"source": "Home", "target": "Inicio", "handling": "translate", "context": "navigation"}],
}


class EnUsToEsMxValidatorTests(unittest.TestCase):
    ...
    def write_controls(self, directory: Path, dictionary=DICTIONARY, voice_profile=VOICE_PROFILE) -> None:
        ...
        (config_root / "dictionary.en-us-es-mx.json").write_text(json.dumps(dictionary), encoding="utf-8")
    ...
    def test_valid_pair_and_protected_tokens_pass(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            target.write_text("# Hola\n\nUtiliza `npm run check` en https://example.test/a.\n\n```js\nconst x = '{name}'\n```\n", encoding="utf-8")
            result = self.run_validator(project, source, target)
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            payload = json.loads(result.stdout)
            self.assertTrue(payload["passed"])
            self.assertEqual(payload["language_pair"]["target_locale"], "es-MX")
    ...
    def test_rejects_any_other_language_pair_or_many_targets(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertTrue(any("es-MX" in item for item in errors))
    ...
    def test_planner_reports_only_es_mx_targets_without_writing(self):
        with tempfile.TemporaryDirectory() as raw:
            directory = Path(raw)
            project = self.write_project(directory)
            self.write_controls(directory)
            ...
            self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            payload = json.loads(result.stdout)
            self.assertEqual(payload["plan"]["language_pair"]["target_locale"], "es-MX")
    ...
    def test_validator_rejects_empty_or_wrong_pair_dictionary(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(result.returncode, 1)
            errors = json.loads(result.stdout)["errors"]
            self.assertTrue(any("en-US to es-MX" in item for item in errors))
