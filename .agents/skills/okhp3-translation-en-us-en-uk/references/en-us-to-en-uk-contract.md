# en-US to en-GB text-artifact contract

This package has one language pair and one direction. The source is plainspoken
`en-US` and the output is `en-GB`. A manifest with another source, target,
...
has a declared en-GB equivalent:
...
- Markdown and HTML link targets remain unchanged unless the project contains
  an explicit en-GB route mapping;
...
Structural parity is not character-count parity. British English may expand or
contract. Do not force unnatural British English to match English length.
...
and claims. Format a value for en-GB only when the project rule says it is safe
...
3. preserve reviewed en-GB units whose source counterpart did not change;
4. translate only changed or new units using the current profile and dictionary;
5. mark the en-GB target stale until checks and review gates pass; and
6. retain the old target or a reviewable diff according to the consuming
   repository's normal version-control policy.

Do not regenerate the whole British English artifact if that would erase a reviewed British English
edit outside the changed source units.

## Publication boundary

A British English artifact does not prove that its route works, its canonical is correct,
