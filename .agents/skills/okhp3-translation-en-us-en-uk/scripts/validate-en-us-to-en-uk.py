#!/usr/bin/env python3
"""Validate one en-US to en-GB text-artifact manifest and optional pair.
...
protected tokens; it cannot certify idiomatic British English or native review.
"""
...
def validate_dictionary(dictionary: Any) -> List[str]:
    ...
    if not isinstance(pair, dict) or pair.get("source_locale") != SOURCE_LOCALE or pair.get("target_locale") != TARGET_LOCALE or pair.get("direction") != "one-way":
        add_error(errors, "target.dictionary must declare exactly the en-US to en-GB one-way pair")
...
def validate_manifest(project: Any, path: Path) -> List[str]:
    ...
    if not isinstance(target, dict):
        add_error(errors, "target must be one object for the en-GB output")
    else:
    ...
    if "targets" in project:
        add_error(errors, "targets is not allowed; this package permits exactly one en-GB output")
...
def compare_pair(source_path: Path, target_path: Path, preserve_urls: bool) -> Tuple[List[str], List[str]]:
    ...
    if not target_path.is_file():
        add_error(errors, f"missing en-GB target file: {target_path}")
        return errors, warnings
    ...
    if source == target:
        warnings.append("target is byte-identical to source; confirm that the British English spelling/vocabulary adaptation was performed")
    if not target.strip():
        add_error(errors, "en-GB target file is empty")
    return errors, warnings
...
def report(errors: List[str], warnings: List[str], output_format: str, project: Path, pair: Optional[Tuple[Path, Path]]) -> int:
    result = {
        ...
        "errors": errors,
        "warnings": warnings,
        "scope": "manifest, completed plainspoken-register gate, structure, and protected-token checks only; no British English quality or native-review certification",
    }
    if output_format == "json":
        print(json.dumps(result, indent=2, ensure_ascii=False))
    else:
        print("PASS" if result["passed"] else "FAIL")
        print("Language pair: en-US -> en-GB")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--project", required=True, type=Path, help="single-pair en-US to en-GB project JSON manifest")
    parser.add_argument("--source-file", type=Path, help="optional en-US source artifact to compare")
    parser.add_argument("--target-file", type=Path, help="optional en-GB target artifact to compare")
