...
| A3 | Site-wide | `sitemap.xml` is missing 5 live pages | Not a WCAG criterion — discoverability/SEO, but affects the "find a specific project" task for the general-public persona | P2 | Older adult / general public, mobile-on-slow-connection (relies on crawler-fed search) | `sitemap.xml` vs. file inventory: `/projects/hometools/`, `/projects/pathscrib-r/`, `/projects/un-nocked-truth/`, `/writings/biases-as-constants/`, `/writings/magnus-saga/` confirmed live, absent from sitemap | **Corrected 2026-08-17:** this was not a regression. All 5 pages carry `<meta name="robots" content="noindex, nofollow">` and sit behind an active "under construction" gate (`data-wip-key="found-ry"`); they are intentionally excluded from indexing/discovery, not "live" in the sense this finding assumed. Confirmed with the site owner to leave them out of `sitemap.xml` as-is. `scripts/validate-site.py` already has a standing check (added independently of this audit) that fails the build if any *non*-noindex page is missing from `sitemap.xml`, and also fails if a noindex page is ever added to it — so both directions of this regression are covered. |
